...
const resultFolderId = document.getElementById('resultFolderId');
const resultSaveControls = document.getElementById('resultSaveControls');
...
function setSaveControlsVisible(visible) {
  if (resultSaveControls) {
    resultSaveControls.style.display = visible ? 'flex' : 'none';
    return;
  }
  if (profitSaveBtn) {
    profitSaveBtn.style.display = visible ? 'inline-block' : 'none';
  }
}
...
function resetInputForm() {
  ...
  if (resultFolderId) {
    resultFolderId.value = resolveDefaultFolderId();
  }
}
...
function normalizeProfitFormRowOrder(order) {
  const base = ['heading', 'itemName', 'cost', 'feeRate', 'shippingId', 'packingId', 'salePrice', 'otherCost', 'actions'];
  if (!Array.isArray(order)) return base;
  const out = [];
...
function renderProfitMoveMode() {
  ...
  const controls = Array.from(
    cardEl.querySelectorAll('input, select, textarea, .movable-form-row button, .movable-result-row button, .result-action-row button')
  );
...
async function refresh() {
  ...
  const prevFolder = resultFolderId?.value || '';
  ...
  fillFolderSelect(resultFolderId, folderOptions);

  if (prevShipping) fields.shippingId.value = prevShipping;
  if (prevPacking) fields.packingId.value = prevPacking;
  if (resultFolderId) {
    if (prevFolder) resultFolderId.value = prevFolder;
    if (!resultFolderId.value && folderOptions.length) resultFolderId.value = resolveDefaultFolderId();
  }
...
profitForm.addEventListener('submit', async (e) => {
  ...
  const payload = {
    ...
    folderId: resultFolderId?.value || resolveDefaultFolderId(),
...
if (resultFolderId) {
  resultFolderId.addEventListener('change', () => {
    if (!pendingSavePayload) return;
    pendingSavePayload.folderId = resultFolderId.value;
    if (!latestResultRecord) return;
    const folder = folderOptions.find((f) => f.id === resultFolderId.value);
    renderResult({
      ...latestResultRecord,
      folderId: resultFolderId.value,
      folderName: folder?.name || latestResultRecord.folderName
    });
  });
}
