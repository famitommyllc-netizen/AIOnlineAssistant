# GAS Full Copy Paste
- Generated: 2026-03-22 22:18:00 JST
- Source dir: apps/erp/src/gas
...
<body>
  <div id="entry-page">
    ...
    <div class="section">
      ...
      <div class="button-row">
        <button type="button" class="btn-main" onclick="showFinalConfirm()">最終確認へ</button>
      </div>
    </div>
  </div>
  ...
  <div id="confirm-page" style="display:none;">
    <div class="section">
      ...
      <div class="button-row">
        ...
        <button type="button" id="confirm-submit-btn" class="btn-main" onclick="openPrintDecisionModal()">仕入登録</button>
        <span id="submit-status"></span>
      </div>
    </div>
  </div>

  <div id="print-decision-modal" class="modal-overlay">
    <div class="modal-card" style="max-width:520px;">
      <span class="modal-close" onclick="closePrintDecisionModal()">×</span>
      <h3 class="mid-title">印刷確認</h3>
      <div class="note-text">仕入登録後に、伝票とバーコードを印刷しますか？</div>
      <div class="button-row" style="margin-top:1em;">
        <button type="button" class="btn-main" onclick="submitEntriesWithPrintChoice(true)">印刷する</button>
        <button type="button" class="btn-sub" onclick="submitEntriesWithPrintChoice(false)">印刷しない</button>
      </div>
    </div>
  </div>
      ...
      <tr>
        ...
        <td>
          <div class="inline-actions">
            <button type="button" class="btn-main" onclick="addEntryRow(${idx})">＋行追加</button>
            <button type="button" class="btn-sub" onclick="removeEntryRow(${idx})">−削除</button>
          </div>
        </td>
      </tr>
    `;
...
function renderPointArea() {
  ...
  const modeRows = `
    <tr>
      ...
      <td class="col-item"><div class="inline-value">入力モード</div></td>
      <td class="col-item">
        <select id="point-mode" class="inline-input" onchange="onPointModeChange(this.value)">
          <option value="allocation" ${isPointAllocationMode_() ? 'selected' : ''}>按分モード</option>
          <option value="individual" ${!isPointAllocationMode_() ? 'selected' : ''}>個別入力モード</option>
        </select>
      </td>
    ...
    <tr>
      ...
      <td class="col-item"><div class="inline-value">ポイント/割引</div></td>
      <td class="col-num">
        ${isPointAllocationMode_()
          ? `<input type="text" id="point-total" class="inline-number" inputmode="numeric" pattern="\\d*" value="${pointTotal || ''}" oninput="updatePointTotal()" placeholder="合計ポイント/割引">`
          : `<div class="inline-value">${pointTotalFromRows}</div>`
        }
      </td>
      <td class="col-action"><div class="inline-value">${isPointAllocationMode_() ? '合計入力' : '商品行で個別入力'}</div></td>
    </tr>
  `;
  document.getElementById('point-area').innerHTML = uiBuildInputTable_({
    title: 'ポイント/割引',
    columns: [
...
function updatePointTotal() {
  pointTotal = Math.max(0, roundYen(document.getElementById('point-total').value || 0));
  recalculateCosts();
}
...
function renderExpenseArea() {
  ...
  document.getElementById('expense-area').innerHTML =
    ...
    uiBuildInputTable_({
      ...
      bodyHtml: `
        <tr>
          ...
          <td class="col-item"><div class="inline-value">入力モード</div></td>
          <td class="col-item">
            <select id="expense-mode" class="inline-input" onchange="onExpenseModeChange(this.value)">
              <option value="allocation" ${isExpenseAllocationMode_() ? 'selected' : ''}>按分モード</option>
              <option value="individual" ${!isExpenseAllocationMode_() ? 'selected' : ''}>個別入力モード</option>
            </select>
          </td>
          <td class="col-action"><div class="inline-value">${isExpenseAllocationMode_() ? '経費明細を商品へ按分' : '商品行の経費欄で個別入力'}</div></td>
        </tr>
      `
    }) +
    uiBuildInputTable_({
      title: isExpenseAllocationMode_() ? '仕入経費入力（原価割合で自動按分）' : '仕入経費入力（明細管理用・任意）',
      columns: [
    ...
    `<div class="note-text">${isExpenseAllocationMode_() ? '按分モード: 経費明細の合計を商品原価割合で自動配分します。' : '個別入力モード: 商品行の「経費個別」欄で直接入力します（この経費明細は記録用として利用可能）。'}</div>`;
}
...
function renderSummary() {
  ...
  const expenseDiff = Math.abs(totalExpenseInput - totalExpenseAlloc);
  const modeNote = (!isExpenseAllocationMode_() && totalExpenseInput > 0 && expenseDiff > 0)
    ? `<div class="note-text">個別入力モード: 経費明細合計(${totalExpenseInput}) と 商品反映合計(${totalExpenseAlloc}) が異なります。</div>`
    : '';

  document.getElementById('summary-area').innerHTML = `
    <h3 class="mid-title">集計</h3>
    <div class="summary-grid">
      <div>ポイント入力モード</div><div>${getPointModeLabel_()}</div>
      <div>経費入力モード</div><div>${getExpenseModeLabel_()}</div>
      <div>商品数</div><div>${totalCount}</div>
      <div>総提示金額</div><div>${totalPrice}</div>
      <div>ポイント合計</div><div>${totalPoint}</div>
      <div>経費入力合計</div><div>${totalExpenseInput}</div>
      <div>経費按分合計</div><div>${totalExpenseAlloc}</div>
      <div>原価合計</div><div>${Math.max(0, totalCost)}</div>
    </div>
    ${modeNote}
  `;
}
...
function renderCommonInfo() {
  ...
  const candidateRows = `
    <tr>
      <td class="col-no">1</td>
      <td class="col-item">
        <select id="candidate-type" class="inline-input" onchange="renderCandidateEditor()">
          <option value="supplier">仕入先</option>
          <option value="payment">支払い方法</option>
        </select>
      </td>
      <td class="col-item"><select id="candidate-value" class="inline-input"></select></td>
      <td class="col-item"><input type="text" id="candidate-add-value" class="inline-input" placeholder="新しい候補を入力"></td>
      <td class="col-action">
        <div class="inline-actions">
          <button type="button" class="btn-main" onclick="addCandidateValue()">＋候補追加</button>
          <button type="button" class="btn-sub" onclick="removeCandidateValue()">−候補削除</button>
        </div>
      </td>
    </tr>
  `;
  ...
  document.getElementById('common-info').innerHTML =
    ...
    '<div class="note-text" id="candidate-status" style="margin-top:0.4em;"></div>';
...
function renderCandidateEditor() {
  const typeEl = document.getElementById('candidate-type');
  const valueEl = document.getElementById('candidate-value');
  if (!typeEl || !valueEl) return;

  const type = typeEl.value;
  const list = type === 'supplier' ? supplierCandidates : paymentCandidates;
  if (!list.length) {
    valueEl.innerHTML = '<option value="">候補なし</option>';
    return;
  }

  valueEl.innerHTML = list.map(v => `<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`).join('');
}

function addCandidateValue() {
  const type = (document.getElementById('candidate-type').value || '').trim();
  const value = (document.getElementById('candidate-add-value').value || '').trim();
  if (!type || !value) {
    alert('編集対象と追加候補を入力してください');
    return;
  }
  ...
  google.script.run
    .withSuccessHandler(function() {
      ...
      document.getElementById('candidate-add-value').value = '';
      loadSupplierPaymentOptions();
    ...
    .addMasterCandidate(type, value);
}

function removeCandidateValue() {
  const type = (document.getElementById('candidate-type').value || '').trim();
  const value = (document.getElementById('candidate-value').value || '').trim();
  if (!type || !value || value === '候補なし') {
  ...
  google.script.run
    ...
    .removeMasterCandidate(type, value);
}
...
function showFinalConfirm() {
  try {
    ...
    const payloadExpenses = getActiveExpenseRows_().map(ex => ({
      name: String(ex.name || '').trim(),
    ...
    activeRows.forEach((row, idx) => {
      productRows += `<tr>
        ...
        <td>${roundYen(row.price)}</td>
        <td>${roundYen(row.point)}</td>
        <td>${roundYen(row.expenseAlloc)}</td>
        <td>${roundYen(row.cost)}</td>
        <td>${roundYen(row.qty)}</td>
        <td>${roundYen(row.unitPrice)}</td>
      </tr>`;
    });
    ...
    if (!payloadExpenses.length) {
      ...
      payloadExpenses.forEach((ex, idx) => {
        expenseRows += `<tr><td>${idx + 1}</td><td>${escapeHtml(ex.name)}</td><td>${ex.amount}</td><td>${escapeHtml(ex.memo)}</td></tr>`;
      });
    }
    ...
    document.getElementById('confirm-area').innerHTML = `
      <div class="confirm-block">
        ...
        <div class="summary-grid">
          <div>仕入日</div><div>${escapeHtml(date)}</div>
          <div>仕入先</div><div>${escapeHtml(supplier)}</div>
          <div>支払い方法</div><div>${escapeHtml(payment)}</div>
          <div>ポイント入力モード</div><div>${escapeHtml(getPointModeLabel_())}</div>
          <div>経費入力モード</div><div>${escapeHtml(getExpenseModeLabel_())}</div>
        </div>
      </div>
      ...
      <div class="confirm-block">
        <div class="confirm-title">経費明細</div>
        <div class="table-wrap">
          <table>
      ...
      <div class="confirm-block">
        ...
        <div class="summary-grid">
          <div>商品数</div><div>${totalQty}</div>
          <div>総提示金額</div><div>${totalPrice}</div>
          <div>ポイント合計</div><div>${totalPoint}</div>
          <div>経費入力合計</div><div>${totalExpenseInput}</div>
          <div>経費按分合計</div><div>${totalExpAlloc}</div>
...
function openPrintDecisionModal() {
  if (!pendingPayload) {
    alert('最終確認を先に実行してください');
    return;
  }
  const modal = document.getElementById('print-decision-modal');
  if (modal) modal.style.display = 'flex';
}

function closePrintDecisionModal() {
  const modal = document.getElementById('print-decision-modal');
  if (modal) modal.style.display = 'none';
}

function submitEntriesWithPrintChoice(printNow) {
  shouldAutoPrintAfterSubmit = !!printNow;
  closePrintDecisionModal();
  submitEntries();
}
...
function buildSlipPrintHtml(data) {
  ...
  const expenseRows = (data.expenses && data.expenses.length)
    ? data.expenses.map(ex => `<tr><td>${ex.lineNo || ''}</td><td>${escapeHtml(ex.name || '')}</td><td>${ex.amount || 0}</td><td>${escapeHtml(ex.memo || '')}</td></tr>`).join('')
    : '<tr><td colspan="4">なし</td></tr>';

  const summaryRows = `
    <tr><th>項目</th><th>値</th></tr>
    <tr><td>総個数</td><td>${(data.totals && data.totals.qty) || 0}</td></tr>
    <tr><td>総提示金額</td><td>${(data.totals && data.totals.price) || 0}</td></tr>
    <tr><td>ポイント合計</td><td>${(data.totals && data.totals.point) || 0}</td></tr>
    <tr><td>経費入力合計</td><td>${(data.totals && data.totals.expenseInputTotal) || 0}</td></tr>
    <tr><td>経費按分合計</td><td>${(data.totals && data.totals.expenseAlloc) || 0}</td></tr>
    <tr><td>原価合計</td><td>${(data.totals && data.totals.cost) || 0}</td></tr>
  `;
...
.summary-table td:first-child { width: 45%; }
  ...
  <div class="page-wrap">
    <div class="left-pane">
      ...
      <div class="info">
        ...
        <div>支払い方法: ${escapeHtml(data.payment || '')}</div>
      </div>
