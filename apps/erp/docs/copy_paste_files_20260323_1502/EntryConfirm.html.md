...
window.onload = function() {
  try {
    draftToken = getQueryParam_('token');
    if (!draftToken) {
...
function renderConfirmSlip_(payload) {
  ...
  const pointModeLabel = String(p.pointMode || 'allocation') === 'individual' ? '個別入力' : '按分入力';
  const expenseModeLabel = String(p.expenseMode || 'allocation') === 'individual' ? '個別入力' : '按分入力';
...
function submitConfirmedEntry() {
  ...
  google.script.run
    .withSuccessHandler(function(res) {
      ...
      window.location.href = buildWebAppPageUrl_('entry', { token: '' });
    })
...
function goBackToEntry() {
  const nextUrl = buildWebAppPageUrl_('entry', { token: draftToken });
